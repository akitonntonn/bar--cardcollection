...
| `upgrade_v2.sql` | **QRコード引換**（redeem_codes）＋管理画面用RPC（検索・コード発行・一覧）。functions.sql の後に Run |
| `upgrade_v3.sql` | **ウェルカム特典**（新規登録でガチャ3回）＋**付与履歴** admin_activity_log()。upgrade_v2.sql の後に Run |
| `upgrade_v4.sql` | **コンプ報酬**（夏限定+5回/NORMAL+3/RARE+2/SR+3）。claim_comp() がサーバ判定で付与。upgrade_v3.sql の後に Run |
